...
const stegUtenNesteKnapp = [
  Steg.START_SØKNAD,
  Steg.DOKUMENTASJON,
  Steg.KVITTERING,
];

export const screenshotPrefix = (testInfo: TestInfo, steg: Steg) =>
  [...testInfo.titlePath.slice(1)].join('-') + `-${stegNummber[steg]}-${steg}`;

export const testSideMedScreenshot = async (
  page: Page,
  testInfo: TestInfo,
  steg: Steg,
  testSide?: (page: Page) => Promise<void>
) => {
  ...
  await expect(page).toHaveScreenshot(`${prefix}-etter.png`, {
    fullPage: true,
  });
